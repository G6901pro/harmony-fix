/** Official Bangladesh administrative dataset: 8 divisions, 64 districts, 494 upazilas/thanas. */
...
export const BD_DISTRICTS: Record<BdDivision, string[]> = {
  ...
  "Chattogram": ["Bandarban", "Brahmanbaria", "Chandpur", "Chattogram", "Coxsbazar", "Cumilla", "Feni", "Khagrachhari", "Lakshmipur", "Noakhali", "Rangamati"],
...
/** District -> Areas / Upazilas / Thanas */
...
/**
 * District -> Metropolitan / City Corporation thanas and commonly used urban areas.
 * Merged on top of the rural upazila list so districts with a city corporation expose both.
 * Add a district key here when a new city corporation / metro police area is created.
 */
export const BD_METRO_THANAS: Record<string, string[]> = {
  "Dhaka": [
    "Adabor", "Airport (Dhaka)", "Badda", "Banani", "Bangshal", "Bhashantek", "Bhatara",
    "Cantonment", "Chawkbazar (Dhaka)", "Dakshinkhan", "Darus Salam", "Demra", "Dhanmondi",
    "Gendaria", "Gulshan", "Hazaribagh", "Jatrabari", "Kadamtali", "Kafrul", "Kalabagan",
    "Kamrangirchar", "Khilgaon", "Khilkhet", "Kotwali (Dhaka)", "Lalbagh", "Mirpur Model",
    "Mohammadpur", "Motijheel", "Mugda", "New Market", "Pallabi", "Paltan", "Ramna",
    "Rampura", "Rupnagar", "Sabujbagh", "Shah Ali", "Shahbagh", "Shahjahanpur",
    "Sher-e-Bangla Nagar", "Shyampur", "Sutrapur", "Tejgaon", "Tejgaon Industrial Area",
    "Turag", "Uttara East", "Uttara West", "Uttarkhan", "Wari",
  ],
  "Chattogram": [
    "Akbarshah", "Bakalia", "Bandar (Chattogram)", "Bayezid Bostami", "Chandgaon",
    "Chawkbazar (Chattogram)", "Double Mooring", "EPZ", "Halishahar", "Khulshi",
    "Kotwali (Chattogram)", "Pahartali", "Panchlaish", "Patenga", "Sadarghat (Chattogram)",
  ],
  "Khulna": [
    "Aranghata", "Daulatpur (Khulna Metro)", "Harintana", "Khalishpur", "Khan Jahan Ali",
    "Khulna Sadar", "Labanchara", "Sonadanga",
  ],
  "Rajshahi": [
    "Airport (Rajshahi)", "Belpukur", "Boalia", "Chandrima", "Damkura", "Karnahar",
    "Kashiadanga", "Katakhali", "Matihar", "Rajpara", "Shah Makhdum",
  ],
  "Sylhet": [
    "Airport (Sylhet)", "Dakshin Surma (Metro)", "Jalalabad", "Kotwali (Sylhet)",
    "Moglabazar", "Shahporan",
  ],
  "Barishal": [
    "Airport (Barishal)", "Bandar (Barishal)", "Barishal Kotwali", "Kawnia (Barishal Metro)",
  ],
  "Rangpur": [
    "Haragachh", "Kotwali (Rangpur)", "Mahiganj", "Parshuram (Rangpur Metro)", "Tajhat",
  ],
  "Mymensingh": ["Kotwali (Mymensingh)"],
  "Gazipur": [
    "Basan", "Gacha", "Gazipur Sadar Metro", "Kashimpur", "Konabari", "Pubail",
    "Tongi East", "Tongi West",
  ],
  "Narayanganj": [
    "Fatullah", "Narayanganj Sadar Model", "Siddhirganj", "Bandar (Narayanganj Metro)",
  ],
  "Cumilla": ["Kotwali Model (Cumilla)", "Sadar Dakshin (Metro)"],
  "Coxsbazar": ["Kolatoli", "Jhilongjha", "Link Road", "Bus Terminal Area", "Laboni Beach Area"],
  "Narsingdi": ["Madhabdi", "Narsingdi Model"],
  "Jashore": ["Jessore Kotwali Model"],
  "Bogura": ["Bogra Kotwali Model"],
  "Dinajpur": ["Kotwali (Dinajpur)"],
  "Faridpur": ["Kotwali (Faridpur)"],
  "Noakhali": ["Sudharam"],
  "Feni": ["Feni Model"],
  "Tangail": ["Tangail Model"],
  "Pabna": ["Pabna Model"],
  "Kushtia": ["Kushtia Model"],
  "Chandpur": ["Chandpur Model"],
  "Brahmanbaria": ["Brahmanbaria Model"],
  "Sirajganj": ["Sirajganj Model"],
};
...
/** All selectable areas for a district: official upazilas + metropolitan thanas / urban areas. */
export function upazilasFor(district: string): string[] {
  const base = BD_UPAZILAS[district] ?? [];
  const metro = BD_METRO_THANAS[district] ?? [];
  return Array.from(new Set([...base, ...metro])).sort((a, b) => a.localeCompare(b));
}

export function divisionForDistrict(district: string): BdDivision | undefined {
  return BD_DIVISIONS.find((d) => BD_DISTRICTS[d].includes(district));
}
