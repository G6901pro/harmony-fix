...
BEGIN
  ...
  -- Only announce on creation, on activation, or when the audience changes.
  IF TG_OP = 'UPDATE'
  ...
  IF NEW.assigned_user_id IS NOT NULL THEN
    -- Private coupon: unchanged behaviour, single recipient.
  ...
  -- Public coupon: every active customer account is notified once.
  INSERT INTO public.notifications (user_id, kind, title, body, link)
...
-- New customers see the public coupons that are live when they join.
